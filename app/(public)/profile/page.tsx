...
import onReadNotify from "@utils/onReadNotify";

export default function ProfilePage() {
  const [data, setData] = useState<User | null | undefined>(undefined);
  const [dataNotifyCount, setDataNotifyCount] = useState<
    NotifyCount | null | undefined
  >(undefined);
  const [dataNotify, setDataNotify] = useState<Notify[] | null | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const dataServNotifyCount = await getNotifyCount();
      const dataServNotify = await getNotify();
      setDataNotifyCount(dataServNotifyCount);
      setDataNotify(dataServNotify);
      setData(dataServ);
    }
  ...
  const handleReadNotify = async (id: string) => {
    await onReadNotify(id);
  };

  const handleClickNotify = () => {
    setActiveNotify((prev) => (prev === "active" ? null : "active"));
  };
          ...
          <section>
            <div className="profile-page__header">
              ...
              <div className="profile-page__header__right-side">
                <div
                  className="profile-page__header__right-side__notify"
                  onClick={handleClickNotify}
                >
                  <div className="profile-page__header__right-side__notify--counter">
                    {dataNotifyCount ? dataNotifyCount.unread_count : "..."}
                  </div>
                    ...
                    : dataNotify.map((elem) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              disabled={elem.is_read}
                              className={`profile-page__header__right-side__notify__section--btn`}
                              onClick={() => handleReadNotify(elem.id)}
                            >
                              {elem.is_read ? (
                                <svg
